...
tags:
  ...
  - 帧同步
  - GamePlay
description: "NKGMobaBasedOnET项目学习笔记 - 锁步帧同步核心组件、命令体系、Ticker系统、一致性检查与回滚完整拆解"
---
...
1. **帧同步 vs 状态同步的本质区别** - 帧同步传操作指令（O(玩家数)），状态同步传完整状态（O(实体数)）
2. **为什么MOBA选帧同步** - 单位多、操作频率低，带宽省3倍以上
3. **客户端预测的作用** - 让玩家觉得操作即时（隐藏网络延迟），但随时准备被服务端纠正
4. **一致性检查的必要性** - 没有它，客户端永远不知道"我预测错了"
5. **浮点数为什么用容差对比** - 浮点运算有精度问题（CPU、编译器、运算顺序）
6. **服务端判脏优化** - 约8%带宽节省，角色站着不动就不发移动命令
7. **逻辑-渲染分离** - 逻辑位置每帧跳变（30fps），用Lerp平滑过渡
8. **手动调FixedUpdate.Tick()** - 精确控制Tick频率，支持追帧和降速
